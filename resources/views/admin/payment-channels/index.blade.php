    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <form action="{{ route('admin.payment-channels.check-all') }}" method="POST" class="d-inline">
                    @csrf
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @forelse($channels->where('type', 'virtual_account') as $channel)
                            <tr>
                                <td>{{ $channel->sort_order }}</td>
                                <td>
                                    <strong>{{ $channel->name }}</strong>
                                    @if($channel->last_checked_at)
                                    <br><small class="text-muted">Last checked: {{ $channel->last_checked_at->diffForHumans() }}</small>
                                    @endif
                                </td>
                                <td><code>{{ $channel->code }}</code></td>
                                <td>
                                    {{ $channel->description }}
                                    @if($channel->partner_service_id)
                                    <br><small class="text-muted"><strong>PSID:</strong> {{ $channel->partner_service_id }}</small>
                                    @endif
                                    @if($channel->last_error)
                                    <br><small class="text-danger"><i class="fa fa-exclamation-triangle"></i> {{ Str::limit($channel->last_error, 50) }}</small>
                                    @endif
                                </td>
                                <td class="text-center">
                                    <form action="{{ route('admin.payment-channels.toggle', $channel) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-sm btn-{{ $channel->is_active ? 'success' : 'secondary' }}">
                                            <i class="fa fa-{{ $channel->is_active ? 'check' : 'times' }}"></i>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editModal{{ $channel->id }}" title="Edit Configuration">
                                        <i class="fa fa-edit"></i>
                                    </button>
                                    <form action="{{ route('admin.payment-channels.check', $channel) }}" method="POST" class="d-inline">
                                        @csrf
                            ...
                            {{-- Edit Modal --}}
                            <div class="modal fade" id="editModal{{ $channel->id }}" tabindex="-1">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('admin.payment-channels.update', $channel) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <div class="modal-header">
                                                <h5 class="modal-title">Edit {{ $channel->name }} Configuration</h5>
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" class="form-control" value="{{ $channel->code }}" disabled>
                                                </div>
                                                <div class="mb-3">
                                                    <label class="form-label">Partner Service ID / BIN <span class="text-danger">*</span></label>
                                                    <input type="text" name="partner_service_id" class="form-control" value="{{ $channel->partner_service_id }}" placeholder="e.g., 13925, 98829172" required>
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <textarea name="bin_notes" class="form-control" rows="2" placeholder="Optional notes about this BIN">{{ $channel->bin_notes }}</textarea>
                                                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @forelse($channels->where('type', 'ewallet') as $channel)
                            <tr>
                                <td>{{ $channel->sort_order }}</td>
                                <td>
                                    <strong>{{ $channel->name }}</strong>
                                        ...
                                        <form action="{{ route('admin.payment-channels.update', $channel) }}" method="POST">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="number" name="sort_order" class="form-control" value="{{ $channel->sort_order }}">
                                                </div>
                                                <div class="alert alert-info small mb-0">
                                                    <i class="fa fa-info-circle"></i>
