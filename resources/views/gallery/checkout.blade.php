@extends('layouts.app')
@section('title', 'Checkout Slot Foto')
@section('page-title', 'Checkout Slot Foto')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
    <li class="breadcrumb-item"><a href="{{ route('gallery.select-quantity') }}">Beli Slot Foto</a></li>
    <li class="breadcrumb-item active">Checkout</li>
@endsection

@push('styles')
<style>
.payment-method-tabs .nav-link {
    border-radius: 8px 8px 0 0;
    font-weight: 500;
}
.payment-method-tabs .nav-link.active {
    background-color: #0d6efd;
    color: white;
    border-color: #0d6efd;
}
.payment-method-tabs .nav-link.disabled {
    opacity: 0.5;
    cursor: not-allowed;
    pointer-events: none;
}
</style>
@endpush

@push('scripts')
<!-- Ensure Bootstrap JS is loaded for tabs -->
<script>
// Initialize Bootstrap tabs if not already initialized
document.addEventListener('DOMContentLoaded', function() {
    // Check if Bootstrap is loaded
    if (typeof bootstrap !== 'undefined') {
        console.log('Bootstrap tabs initialized');
    } else {
        console.warn('Bootstrap JS not loaded - tabs may not work');
    }
});
</script>
@endpush

@section('content')
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header text-center bg-primary text-white">
                <h4 class="card-title mb-0">Pembayaran Slot Foto Gallery</h4>
            </div>
            <div class="card-body">
                {{-- Order Summary --}}
                <div class="row mb-4">
                    <div class="col-md-6">
                        <div class="text-center mb-3">
                            <span class="badge bg-warning text-dark" style="font-size:1.1rem;padding:8px 20px">
                                <i class="fa fa-images"></i> Gallery Pool
                            </span>
                        </div>

                        <table class="table table-sm">
                            <tr><td class="text-muted">No. Order</td><td class="fw-bold">{{ $order->order_number }}</td></tr>
                            <tr><td class="text-muted">Item</td><td>Slot Foto Tambahan</td></tr>
                            <tr><td class="text-muted">Jumlah</td><td class="fw-bold">{{ $qty }} slot</td></tr>
                            <tr><td class="text-muted">Harga per slot</td><td>Rp {{ number_format($pricePerPhoto, 0, ',', '.') }}</td></tr>
                            <tr class="table-active">
                                <td class="fw-bold">Total Pembayaran</td>
                                <td class="fw-bold text-success fs-5">Rp {{ number_format($subtotal, 0, ',', '.') }}</td>
                            </tr>
                        </table>

                        <div class="alert alert-info small">
                            <i class="fa fa-info-circle"></i>
                            Slot foto akan masuk ke gallery pool Anda dan bisa digunakan di semua undangan.
                        </div>
                    </div>

                    <div class="col-md-6">
                        @php
                            $hasActiveVA = isset($virtualAccount) && $virtualAccount;
                            $hasActiveQris = isset($qrisPayment) && $qrisPayment;
                            $hasAnyActivePayment = $hasActiveVA || $hasActiveQris;
                        @endphp

                        @if($hasAnyActivePayment)
                        <div class="alert alert-warning mb-3">
                            <i class="fa fa-info-circle"></i>
                            <strong>Perhatian:</strong> Anda sudah memiliki pembayaran yang aktif. Selesaikan pembayaran tersebut terlebih dahulu.
                        </div>
                        @endif

                        {{-- Payment Method Tabs --}}
                        <ul class="nav nav-tabs payment-method-tabs mb-3" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link {{ $hasActiveVA || !$hasAnyActivePayment ? 'active' : '' }} {{ $hasAnyActivePayment && !$hasActiveVA ? 'disabled' : '' }}" 
                                        id="va-tab" 
                                        data-bs-toggle="tab" 
                                        data-bs-target="#va-content" 
                                        type="button" 
                                        role="tab"
                                        {{ $hasAnyActivePayment && !$hasActiveVA ? 'disabled' : '' }}>
                                    <i class="fa fa-university"></i> Virtual Account
                                </button>
                            </li>
                            @if(isset($qrisEnabled) && $qrisEnabled)
                            <li class="nav-item" role="presentation">
                                <button class="nav-link {{ $hasActiveQris ? 'active' : '' }} {{ $hasAnyActivePayment && !$hasActiveQris ? 'disabled' : '' }}" 
                                        id="qris-tab" 
                                        data-bs-toggle="tab" 
                                        data-bs-target="#qris-content" 
                                        type="button" 
                                        role="tab"
                                        {{ $hasAnyActivePayment && !$hasActiveQris ? 'disabled' : '' }}>
                                    <i class="fa fa-qrcode"></i> QRIS
                                </button>
                            </li>
                            @else
                            <li class="nav-item" role="presentation">
                                <button class="nav-link disabled" type="button" disabled>
                                    <i class="fa fa-qrcode"></i> QRIS (Belum Aktif)
                                </button>
                            </li>
                            @endif
                        </ul>

                        <div class="tab-content">
                            {{-- Virtual Account Tab --}}
                            <div class="tab-pane fade {{ $hasActiveVA || !$hasAnyActivePayment ? 'show active' : '' }}" id="va-content" role="tabpanel">
                                @if(isset($virtualAccount) && $virtualAccount)
                                {{-- Virtual Account Info --}}
                                <div class="alert alert-success">
                                    <h6 class="alert-heading mb-3"><i class="fa fa-check-circle"></i> Virtual Account Berhasil Dibuat</h6>
                                    
                                    <div class="mb-3">
                                        <label class="text-muted small d-block mb-1">Bank</label>
                                        <div class="fw-bold fs-5">
                                            @php
                                                $bankName = match($virtualAccount->channel) {
                                                    'VIRTUAL_ACCOUNT_CIMB' => 'CIMB Niaga',
                                                    'VIRTUAL_ACCOUNT_MANDIRI' => 'Mandiri',
                                                    'VIRTUAL_ACCOUNT_BRI' => 'BRI',
                                                    'VIRTUAL_ACCOUNT_BNI' => 'BNI',
                                                    'VIRTUAL_ACCOUNT_PERMATA' => 'Permata',
                                                    default => 'Bank'
                                                };
                                            @endphp
                                            {{ $bankName }}
                                        </div>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="text-muted small d-block mb-1">Nomor Virtual Account</label>
                                        <div class="input-group input-group-lg">
                                            <input type="text" class="form-control fw-bold text-center" 
                                                   value="{{ $virtualAccount->display_va_number }}" 
                                                   id="vaNumber" readonly 
                                                   style="font-size: 1.25rem; letter-spacing: 2px;">
                                            <button class="btn btn-outline-success" type="button" onclick="copyVA()">
                                                <i class="fa fa-copy"></i> Salin
                                            </button>
                                        </div>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="text-muted small d-block mb-1">Jumlah</label>
                                        <div class="fw-bold text-success" style="font-size: 1.5rem;">
                                            Rp {{ number_format($virtualAccount->amount, 0, ',', '.') }}
                                        </div>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="text-muted small d-block mb-1">Berlaku Sampai</label>
                                        <div class="fw-bold text-danger fs-6">
                                            {{ $virtualAccount->expired_at->format('d M Y H:i') }} WIB
                                        </div>
                                    </div>
                                    
                                    <hr>
                                    
                                    <div class="d-grid gap-2 mb-3">
                                        <button type="button" class="btn btn-primary btn-lg" onclick="checkPaymentStatus()" id="checkStatusBtn">
                                            <i class="fa fa-refresh"></i> Cek Status Pembayaran
                                        </button>
                                    </div>
                                    
                                    <div class="small mb-0">
                                        <i class="fa fa-info-circle"></i>
                                        Transfer sesuai nominal <strong>exact</strong>. Pembayaran akan diverifikasi otomatis dalam 1-5 menit.
                                    </div>
                                </div>
                                @else
                                {{-- Create VA Form --}}
                                <div class="card">
                                    <div class="card-header">
                                        <h6 class="mb-0">Buat Virtual Account</h6>
                                    </div>
                                    <div class="card-body">
                                        <form action="{{ route('gallery.create-va', $order) }}" method="POST">
                                            @csrf
                                            
                                            <div class="mb-3">
                                                <label class="form-label">Nomor Telepon <span class="text-danger">*</span></label>
                                                <input type="text" 
                                                       name="phone" 
                                                       class="form-control @error('phone') is-invalid @enderror" 
                                                       value="{{ old('phone', auth()->user()->phone) }}"
                                                       placeholder="Contoh: 081234567890"
                                                       required>
                                                @error('phone')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                                <small class="form-text text-muted">
                                                    Format: 08xxx atau 628xxx (9-15 digit)
                                                </small>
                                            </div>
                                            
                                            <div class="mb-3">
                                                <label class="form-label">Bank Virtual Account <span class="text-danger">*</span></label>
                                                <select name="channel" class="form-select" required>
                                                    <option value="">-- Pilih Bank --</option>
                                                    @foreach(\App\Services\DokuVirtualAccountService::getAvailableChannels() as $code => $name)
                                                    <option value="{{ $code }}">{{ $name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            
                                            <button type="submit" class="btn btn-primary w-100">
                                                <i class="fa fa-credit-card"></i> Buat Virtual Account
                                            </button>
                                        </form>
                                    </div>
                                </div>
                                @endif
                            </div>

                            {{-- QRIS Tab --}}
                            <div class="tab-pane fade {{ $hasActiveQris ? 'show active' : '' }}" id="qris-content" role="tabpanel">
                                @if(isset($qrisPayment) && $qrisPayment)
                                {{-- QRIS Payment Info --}}
                                <div class="card border-primary">
                                    <div class="card-header bg-primary text-white">
                                        <h6 class="mb-0"><i class="fa fa-qrcode"></i> Pembayaran QRIS</h6>
                                    </div>
                                    <div class="card-body text-center">
                                        @if($qrisPayment->qr_content)
                                        <div class="mb-3">
                                            <div id="qrcode" class="d-inline-block p-3 bg-white border rounded"></div>
                                        </div>
                                        @endif
                                        
                                        <div class="mb-3">
                                            <label class="text-muted small">Jumlah Pembayaran</label>
                                            <div class="fw-bold text-success" style="font-size: 1.75rem;">
                                                {{ $qrisPayment->formatted_amount }}
                                            </div>
                                        </div>
                                        
                                        <div class="mb-3">
                                            <label class="text-muted small">Status</label>
                                            <div>
                                                @if($qrisPayment->status === 'pending')
                                                    <span class="badge bg-warning">Menunggu Pembayaran</span>
                                                @elseif($qrisPayment->status === 'paid')
                                                    <span class="badge bg-success">Berhasil</span>
                                                @else
                                                    <span class="badge bg-secondary">{{ ucfirst($qrisPayment->status) }}</span>
                                                @endif
                                            </div>
                                        </div>
                                        
                                        <div class="alert alert-info small">
                                            <i class="fa fa-info-circle"></i>
                                            <strong>Cara Pembayaran:</strong><br>
                                            1. Buka aplikasi e-wallet atau mobile banking<br>
                                            2. Pilih menu Scan QR atau QRIS<br>
                                            3. Scan kode QR di atas<br>
                                            4. Konfirmasi pembayaran<br>
                                            5. Pembayaran akan diverifikasi otomatis
                                        </div>
                                        
                                        <div class="alert alert-warning small mb-3">
                                            <i class="fa fa-clock"></i>
                                            Berlaku sampai: <strong>{{ $qrisPayment->expired_at->format('d M Y H:i') }} WIB</strong>
                                        </div>
                                        
                                        <div class="d-grid gap-2">
                                            <button type="button" class="btn btn-primary" onclick="checkQrisStatus()" id="checkQrisBtn">
                                                <i class="fa fa-refresh"></i> Cek Status Pembayaran
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                @else
                                {{-- Create QRIS Payment Form --}}
                                <div class="card">
                                    <div class="card-header">
                                        <h6 class="mb-0">Buat Pembayaran QRIS</h6>
                                    </div>
                                    <div class="card-body">
                                        <div class="alert alert-info small mb-3">
                                            <i class="fa fa-qrcode"></i>
                                            <strong>QRIS</strong> adalah metode pembayaran yang dapat digunakan di semua aplikasi e-wallet dan mobile banking yang mendukung QRIS.
                                        </div>
                                        
                                        <form id="qris-form">
                                            @csrf
                                            
                                            <div class="mb-3">
                                                <label class="form-label">Nomor Telepon <span class="text-danger">*</span></label>
                                                <input type="text" 
                                                       name="phone" 
                                                       id="qris-phone"
                                                       class="form-control" 
                                                       value="{{ old('phone', auth()->user()->phone) }}"
                                                       placeholder="Contoh: 081234567890"
                                                       required>
                                                <small class="form-text text-muted">
                                                    Nomor telepon untuk notifikasi pembayaran
                                                </small>
                                            </div>
                                            
                                            <button type="submit" class="btn btn-primary w-100" id="qris-submit-btn">
                                                <i class="fa fa-qrcode"></i> Buat Kode QRIS
                                            </button>
                                        </form>
                                    </div>
                                </div>
                                @endif
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

@if(isset($virtualAccount) && $virtualAccount)
<script>
function copyVA() {
    const vaInput = document.getElementById('vaNumber');
    vaInput.select();
    document.execCommand('copy');
    
    const btn = event.target.closest('button');
    const originalHTML = btn.innerHTML;
    btn.innerHTML = '<i class="fa fa-check"></i> Copied!';
    btn.classList.add('btn-success');
    btn.classList.remove('btn-outline-success');
    
    setTimeout(() => {
        btn.innerHTML = originalHTML;
        btn.classList.remove('btn-success');
        btn.classList.add('btn-outline-success');
    }, 2000);
}

function checkPaymentStatus() {
    const btn = document.getElementById('checkStatusBtn');
    const originalHTML = btn.innerHTML;
    
    btn.disabled = true;
    btn.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Mengecek...';
    
    fetch('{{ route("api.payment.check-va-status") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
        },
        body: JSON.stringify({
            va_id: {{ $virtualAccount->id }}
        })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success && data.status === 'paid') {
            Swal.fire({
                icon: 'success',
                title: 'Pembayaran Berhasil!',
                text: 'Halaman akan di-refresh...',
                timer: 2000,
                timerProgressBar: true,
                showConfirmButton: false,
            }).then(() => {
                window.location.reload();
            });
        } else {
            Swal.fire({
                icon: 'info',
                title: 'Belum Ada Pembayaran',
                text: data.message || 'Pembayaran belum diterima',
                confirmButtonText: 'OK',
            });
        }
    })
    .catch(error => {
        console.error('Error:', error);
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Terjadi kesalahan saat mengecek status',
            confirmButtonText: 'OK',
        });
    })
    .finally(() => {
        btn.disabled = false;
        btn.innerHTML = originalHTML;
    });
}

// Auto check every 30 seconds
setInterval(() => {
    checkPaymentStatus();
}, 30000);
</script>
@endif

@if(isset($qrisPayment) && $qrisPayment && $qrisPayment->qr_content)
<script src="https://cdn.jsdelivr.net/npm/qrcodejs@1.0.0/qrcode.min.js"></script>
<script>
document.addEventListener('DOMContentLoaded', function() {
    new QRCode(document.getElementById("qrcode"), {
        text: "{{ $qrisPayment->qr_content }}",
        width: 256,
        height: 256,
        colorDark : "#000000",
        colorLight : "#ffffff",
        correctLevel : QRCode.CorrectLevel.H
    });
});

function checkQrisStatus() {
    const btn = document.getElementById('checkQrisBtn');
    const originalHTML = btn.innerHTML;
    
    btn.disabled = true;
    btn.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Mengecek...';
    
    fetch('{{ route("api.payment.check-qris-status") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
        },
        body: JSON.stringify({
            qris_id: {{ $qrisPayment->id }}
        })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success && data.status === 'paid') {
            Swal.fire({
                icon: 'success',
                title: 'Pembayaran Berhasil!',
                text: 'Halaman akan di-refresh...',
                timer: 2000,
                timerProgressBar: true,
                showConfirmButton: false,
            }).then(() => {
                window.location.reload();
            });
        } else {
            Swal.fire({
                icon: 'info',
                title: 'Belum Ada Pembayaran',
                text: data.message || 'Pembayaran belum diterima',
                confirmButtonText: 'OK',
            });
        }
    })
    .catch(error => {
        console.error('Error:', error);
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Terjadi kesalahan saat mengecek status',
            confirmButtonText: 'OK',
        });
    })
    .finally(() => {
        btn.disabled = false;
        btn.innerHTML = originalHTML;
    });
}

// Auto check every 30 seconds
setInterval(() => {
    checkQrisStatus();
}, 30000);
</script>
@endif

<script>
// Handle QRIS form submission with AJAX
document.getElementById('qris-form')?.addEventListener('submit', function(e) {
    e.preventDefault();
    
    const btn = document.getElementById('qris-submit-btn');
    const originalHTML = btn.innerHTML;
    
    btn.disabled = true;
    btn.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Membuat QRIS...';
    
    const phone = document.getElementById('qris-phone').value;
    
    fetch('{{ route("gallery.create-qris", $order) }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
        },
        body: JSON.stringify({
            phone: phone
        })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            // Show success notification
            Swal.fire({
                icon: 'success',
                title: 'QRIS Berhasil Dibuat!',
                text: 'Halaman akan di-refresh untuk menampilkan kode QR...',
                timer: 2000,
                timerProgressBar: true,
                showConfirmButton: false,
            }).then(() => {
                window.location.href = '{{ route("gallery.checkout") }}?qty={{ $order->qty }}&order_id={{ $order->id }}';
            });
        } else {
            // Show error notification
            Swal.fire({
                icon: 'error',
                title: 'Gagal Membuat QRIS',
                text: data.message || 'Terjadi kesalahan saat membuat QRIS',
                confirmButtonText: 'OK',
            });
            btn.disabled = false;
            btn.innerHTML = originalHTML;
        }
    })
    .catch(error => {
        console.error('Error:', error);
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Terjadi kesalahan saat membuat QRIS',
            confirmButtonText: 'OK',
        });
        btn.disabled = false;
        btn.innerHTML = originalHTML;
    });
});
</script>
@endsection
