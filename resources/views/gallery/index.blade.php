    ...
    <div class="card-body">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            ...
            <div class="d-flex gap-2 align-items-center">
                ...
                <a href="{{ route('gallery.select-quantity') }}" class="btn btn-warning btn-sm">
        ...
        <div class="card sticky-top" style="top: 20px;">
            ...
            <div class="card-body">
                @if($remaining <= 0)
                    ...
                    <a href="{{ route('gallery.select-quantity') }}" class="btn btn-warning w-100">
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">Gallery Pool ({{ $allUserPhotos->count() }} foto)</h5>
                <span class="badge bg-primary">{{ count($selectedPhotoIds) }} dipilih untuk undangan ini</span>
            </div>
            <div class="card-body">
                ...
                @else
                    <div class="row g-3" id="photoGrid">
                        ...
                        <div class="col-md-4 col-sm-6">
                            <div class="photo-item {{ in_array($photo->id, $selectedPhotoIds) ? 'selected' : '' }}" 
                                 data-photo-id="{{ $photo->id }}"
                                 data-selected="{{ in_array($photo->id, $selectedPhotoIds) ? 'true' : 'false' }}">
                                <img src="{{ $photo->url }}" alt="{{ $photo->caption }}" class="rounded">
                                <div class="selection-badge"></div>
                                <div class="p-2">
                                    @if($photo->caption)
                                        <small class="text-muted d-block">{{ $photo->caption }}</small>
                                    @endif
...
@push('scripts')
<script>
let selectedPhotos = @json($selectedPhotoIds);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('saveSelectionBtn').addEventListener('click', function() {
        ...
        // Send to server
        fetch('{{ route("invitations.gallery.select", $invitation) }}', {
            method: 'POST',
            headers: {
