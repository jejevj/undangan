{{--
    Partial Guest Messages Section — dipakai oleh semua template undangan.
    Variabel: $invitation, $guestName (from 'to' parameter)
--}}
@if(isset($guestName) && $guestName)
<section id="pesan" class="section {{ $messageSectionClass ?? '' }} reveal">
    ...
    {{-- Daftar Pesan dari Tamu Lain --}}
    @if($invitation->guestMessages->count() > 0)
    <div class="guest-messages-list" style="max-width:700px;margin:0 auto">
        <h3 style="font-size:1.1rem;margin-bottom:20px;color:var(--brown)">
            Ucapan dari Tamu ({{ $invitation->guestMessages->count() }})
        </h3>
        <div class="messages-container" id="messagesContainer">
            @foreach($invitation->guestMessages as $msg)
            <div class="message-card" data-message-id="{{ $msg->id }}">
                <div class="message-header">
                    <div class="message-avatar">{{ $msg->initials }}</div>
                    <div class="message-meta">
                        <div class="message-name">{{ $msg->formatted_name }}</div>
                        <div class="message-date">{{ $msg->created_at->diffForHumans() }}</div>
                    </div>
                </div>
                <div class="message-body">{{ $msg->message }}</div>
                <div class="message-footer">
                    <button class="btn-like" onclick="likeMessage({{ $msg->id }}, this)" data-likes="{{ $msg->likes_count ?? 0 }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            fetch('{{ route("invitation.guest-message.store", $invitation->slug) }}', {
                method: 'POST',
                headers: {
...
function likeMessage(messageId, button) {
    ...
    fetch('{{ route("invitation.guest-message.like", ["slug" => $invitation->slug, "message" => ":id"]) }}'.replace(':id', messageId), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
