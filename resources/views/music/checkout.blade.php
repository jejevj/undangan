@extends('layouts.app')
@section('title', 'Checkout Slot Upload Musik')
@section('page-title', 'Checkout Slot Upload Musik')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
    <li class="breadcrumb-item"><a href="{{ route('music.slots.buy') }}">Beli Slot Upload Musik</a></li>
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        <table class="table table-sm">
                            ...
                            <tr><td class="text-muted">Harga per slot</td><td>Rp {{ number_format($pricePerSlot, 0, ',', '.') }}</td></tr>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade {{ $hasActiveVA || !$hasAnyActivePayment ? 'show active' : '' }}" id="va-content" role="tabpanel">
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <form action="{{ route('music.slots.create-va', $order) }}" method="POST">
                                            @csrf
...
function checkPaymentStatus() {
    ...
    fetch('{{ route("music.slots.check-status", $order) }}', {
        method: 'GET',
        headers: {
    ...
    .then(data => {
        if (data.status === 'paid') {
            Swal.fire({
                ...
                window.location.href = data.redirect || '{{ route("music.upload") }}';
            });
        } else {
            Swal.fire({
                icon: 'info',
...
document.getElementById('qris-form')?.addEventListener('submit', function(e) {
    ...
    fetch('{{ route("music.slots.create-qris", $order) }}', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            Swal.fire({
                ...
                window.location.href = '{{ route("music.slots.checkout") }}?qty={{ $order->qty }}&order_id={{ $order->id }}';
            });
        } else {
            Swal.fire({
                icon: 'error',
